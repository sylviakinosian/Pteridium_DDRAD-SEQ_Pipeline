...
avg_k <- function(kval, n_inds = 97, k_file_1, k_file_2, k_file_3, ...){
	num <- (nargs() - 2)
	df <- as.data.frame(matrix(nrow = (num + 1),  ncol = n_inds))
	for (i in 1:num){
		df[,i] <- k_file_i[,1]
	}
	df[,(num+1)] <- rowMeans(df[sapply(df, is.numeric)])
	return(df)
}

# makes a data frame with your averaged chains
make_data_frame <- function(k_file, n_inds, k_val, names){
	col = k_val + 1
	df <- as.data.frame(matrix(nrow = n_inds, ncol = col))
	x = 1
	for (i in 1:k_val){
		for (j in seq(from=n_inds, to=(n_inds*k_val), by=n_inds)){
			df[,i] <- k_file[x:j,1]
			x <- x + n_inds
		}
	}
